...
public class numberProcessor implements PageProcessor {
    private Site site = Site.me().setRetryTimes(5).setSleepTime(100).setTimeOut(12000);
    @Override
    public void process(Page page) {
        String name = page.getHtml().xpath("//h1[@itemprop='headline']/text()").toString();
        if (page.getUrl().toString().indexOf("エピソード")!=-1){
            int size = page.getHtml().xpath("//table[@id='content_block_5']/tbody/tr").all().size()-1;
            ...
            int size = 0;
            List<String> star = page.getHtml().xpath("//table[@id='content_block_4']/tbody/tr/td[1]/text()").all();
